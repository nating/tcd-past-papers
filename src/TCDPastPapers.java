...
public class TCDPastPapers {
    ...
    private static void writeDBToJSON(ArrayList<Course> courses, ArrayList<Module> modules,String json){
        ...
        JSONObject listOfCourseNames = new JSONObject();
        for(int i=0;i<courses.size();i++){
            JSONObject course = new JSONObject();
            course.put("name",courses.get(i).getName());
            listOfCourseNames.put(""+courses.get(i).getSearchValue(),course);
        }
        ...
        for(int i=0;i<modules.size();i++){
            ...
            m.put("papers",papers);
            if(!module.getID().equals("") && !module.getID().contains("$") && !module.getID().contains("#") && !module.getID().contains("]")  && !module.getID().contains("[") && !module.getID().contains(".") && !module.getID().contains("/")) {
        ...
        for(int i=0;i<courses.size();i++){
            ...
            listOfCourses.put(""+courses.get(i).getSearchValue(),years);
        }
